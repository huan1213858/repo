...
import com.imp.pojo.Group;
import com.imp.pojo.Product;
import com.imp.pojo.Provider;
...
import com.imp.service.SaleService;
...
//        System.out.println("*********" + products);
    ...
    public String toAddProductPage(Model model) {
        // 2021年4月30日01:02:57 新增 attribute(为了让添加商
        //        // 2021年4月30日03:21:12(测试用)品信息时, 显示出商品分类信息而不是分类id)
        List<Group> allGroups = groupService.getAllGroups();
        model.addAttribute("allGroups", allGroups);
    ...
    public String checkServicePlusPro(HttpServletRequest request, Model model) {
        System.out.println("checkServicePlusPro-------start--");

        if (request.getParameter("productId") == null||request.getParameter("productId")==""){
            model.addAttribute("ERROR", "请输入商品编号");
            return "other/checkout";
        }else if (request.getParameter("productNumber")==null||request.getParameter("productNumber")==""){
            model.addAttribute("ERROR", "请输入购买数量");
            return "other/checkout";
        }
        // 从表单获取销售商品的id
        Integer productId = Integer.parseInt(request.getParameter("productId"));
        ...
        System.out.println("checkServicePlusPro中product======================" + product);
        if(product == null||checkObj==null){
            model.addAttribute("ERROR", "请重新输入商品编号");
        }else {
            // 在这里完成最后对Check的初始化
            checkObj.setProductNumber(Integer.parseInt(request.getParameter("productNumber")));
            ...
            if (checkObj.getProductNumber() > product.getStock()) {
                model.addAttribute("ERROR", "商品库存不足, 请联系管理员增加库存！");
            } else {
                checkObj.setCheckPrice(checkObj.getProductNumber() * checkObj.getProductSalePrice());
                model.addAttribute("checkObj", checkObj);
...
//        productService.getProductSalePriceByProductId()
    ...
    public String updateProductByProductId(Product product) {
        System.out.println("修改后数据" + product);
